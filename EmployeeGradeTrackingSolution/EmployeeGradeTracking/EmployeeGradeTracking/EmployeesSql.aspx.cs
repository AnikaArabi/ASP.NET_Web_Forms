...
{
    public partial class EmployeesSql : System.Web.UI.Page
    {
        ...
        {
            var id = DataBinder.Eval(e.Item.DataItem, "DepartmentId").ToString();
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["cs"].ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand($"SELECT DepartmentName FROM Departments WHERE DepartmentId={id}", con))
                {
                    con.Open();
                    var dr = cmd.ExecuteReader();
                    if (dr.Read())
                    {
                        var l = e.Item.FindControl("IdLabel") as Label;
                        if (l != null)
